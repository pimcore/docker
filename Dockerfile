...
RUN set -eux; \
    ...
    apt-get install -y \
        ...
        libmagickwand-dev \
    ; \
    # Harden ImageMagick against untrusted SVG assets (GHSA-m656-jmhm-hfwv LFI via text:, GHSA-37r9-mm26-rw8f RCE via msl:): deny the coders an SVG xlink:href can invoke (path is ImageMagick-6 on this Debian)
